...
namespace firestarter::environment::x86::platform {
    ...
    class X86PlatformConfig : public environment::platform::PlatformConfig {
		...
		public:
			X86PlatformConfig(std::string name, unsigned family, std::initializer_list<unsigned> models, std::initializer_list<unsigned> threads,
					std::initializer_list<unsigned> dataCacheBufferSize, unsigned ramBuffersize,
					unsigned currentFamily, unsigned currentModel, unsigned currentThreads, payload::X86Payload *payload) :
			    PlatformConfig(name, threads, dataCacheBufferSize, ramBuffersize, payload), _family(family), _models(models), _currentFamily(currentFamily), _currentModel(currentModel), _currentThreads(currentThreads) {};
