...
int AArch64DefaultPayload::compilePayload(
  ...
  auto l3_addr = x3;
  auto ram_addr = x4;
  auto l2_count_reg = x5;
  auto l3_count_reg = x6;
  auto ram_count_reg = x7;
  auto temp_reg = x8;
  auto temp_reg2 = x9;
  auto offset_reg = x10;
  ...
  // load address to temp register
  cb.ldr(temp_reg, ptr(addrHigh_reg));
  cb.cbz(temp_reg,FunctionExit);
  ...
  if (trans_regs > 0) {
    ...
    for (int i = trans_start + 1; i <= trans_end; i++) {
      if (i % 2 == 0) {
        cb.lsr(temp_reg, temp_reg, Imm(4));
      } else {
  ...
  cb.add(l2_addr, l2_addr, Imm(l1_size)); // address for L2-buffer
  cb.mov(l3_addr, pointer_reg);
  cb.add(l3_addr, l3_addr, Imm(l2_size)); // address for L3-buffer
  cb.mov(ram_addr, pointer_reg);
  cb.add(ram_addr, ram_addr, Imm(l3_size)); // address for RAM-buffer
...
#define L1_INCREMENT()                                                         \
  ...
  if (l1_offset < l1_size * 0.5) {                                             \
    cb.add(l1_addr, l1_addr, offset_reg);                                               \
  } else {                                                                     \
    l1_offset = 0;                                                             \
    cb.mov(l1_addr, pointer_reg);                                              \
  }

#define L2_INCREMENT() cb.add(l2_addr, l2_addr, offset_reg);

#define L3_INCREMENT() cb.add(l3_addr, l3_addr, offset_reg)
...
#define RAM_INCREMENT() cb.add(ram_addr, ram_addr, offset_reg)
  ...
  if (this->getRAMSequenceCount(sequence) > 0) {
    ...
    cb.sub(ram_count_reg, ram_count_reg, Imm(1));
    cb.cbnz(ram_count_reg, NoRamReset);
    cb.mov(ram_count_reg, Imm(ram_loop_count));
    cb.mov(ram_addr, pointer_reg);
    cb.add(ram_addr, ram_addr, Imm(l3_size));
    cb.bind(NoRamReset);
  ...
  if (this->getL2SequenceCount(sequence) > 0) {
    ...
    cb.sub(l2_count_reg, l2_count_reg, Imm(1));
    cb.cbnz(l2_count_reg, NoL2Reset);
    cb.mov(l2_count_reg, Imm(l2_loop_count));
    cb.mov(l2_addr, pointer_reg);
    cb.add(l2_addr, l2_addr, Imm(l1_size));
    cb.bind(NoL2Reset);
    // adds always two instruction
    this->_instructions += 2;
  }
  if (this->getL3SequenceCount(sequence) > 0) {
    ...
    cb.sub(l3_count_reg, l3_count_reg, Imm(1));
    cb.cbnz(l3_count_reg, NoL3Reset);
    cb.mov(l3_count_reg, Imm(l3_loop_count));
    cb.mov(l3_addr, pointer_reg);
    cb.add(l3_addr, l3_addr, Imm(l2_size));
    cb.bind(NoL3Reset);
    // adds always two instruction
    this->_instructions += 2;
  }
  cb.add(iter_reg, iter_reg, Imm(1)); // increment iteration counter
  cb.mov(l1_addr, pointer_reg);
...
*/
  ...
  cb.mov(x0, iter_reg); // restore iteration counter
